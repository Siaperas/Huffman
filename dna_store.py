...
def parse_args():
    ...
    parser.add_argument('-d', default=None,action='store_true', help='Decoding.')
...
if decoding==None:
    ...
    while(len(nodes2)>0):
        if(len(nodes2)>=3):
            ...
            new_node = Node(number=summ,child_1=nodes2[0], child_2=nodes2[1], child_3=nodes2[2])            
            nodes2.pop(0)
            nodes2.pop(0)
            nodes2.pop(0)
    ...
    for i in result:
        check=check+i
        try:
            finalresult=finalresult+list(huffmandict.keys())[list(huffmandict.values()).index(check)]
            check=""
        except ValueError:
            aaaa=1
    f = open(outputfile, 'w')
